...
//export ZipUint64Pair
func ZipUint64Pair(big, lil uint64) (*C.char, int) {
	zipped := zipUint64Pair(big, lil)
	return toCBytes(zipped)
}

//export UnzipUint64Pair
func UnzipUint64Pair(cZip *C.cchar_t, szZip int) (big, lil uint64) {
	zipped := charToBytes(cZip, szZip)
	big, lil = unzipUint64Pair(zipped)
	return
}

//export ZipIntUint64Pair
func ZipIntUint64Pair(i int64, u uint64) (*C.char, int) {
	z := zigZagInt64(i)
	return ZipUint64Pair(z, u)
}

//export UnzipIntUint64Pair
func UnzipIntUint64Pair(cZip *C.cchar_t, szZip int) (i int64, u uint64) {
	zipped := charToBytes(cZip, szZip)
	z, u := unzipUint64Pair(zipped)
	i = zagZigUint64(z)
	return
}
