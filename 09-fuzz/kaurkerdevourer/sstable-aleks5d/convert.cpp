...
void parseBin(string filename) {
    ...
    if (!input) {
        Exit1("can't open " + filename + ".bin file for reading");
    }

    auto output_sstable = fopen((filename + ".sstable").c_str(), "w");
    if (!output_sstable) {
        Exit1("can't open " + filename + ".sstable file for writing");
    }

    auto output_index = fopen((filename + ".index").c_str(), "w");
    if (!output_index) {
        Exit1("can't open " + filename + ".index file for writing");
    }
...
void parseSstable(string sstableName, string indexName) {
    ...
    if (!input_sstable) {
        Exit1("can't open " + sstableName + ".sstable for reading");
    }
    
    auto input_index = fopen((indexName + ".index").c_str(), "r");
    if (!input_index) {
        Exit1("can't open " + indexName + ".index for reading");
    }

    auto output = fopen((sstableName + "_output.bin").c_str(), "w");
    if (!output) {
        Exit1("can't open " + sstableName + "_output.bin for writing");
    }
    ...
    cout << "bin file saved to " << sstableName + "_output.bin";
    cout << endl;
    fclose(input_sstable);
    fclose(input_index);
    fclose(output);
}
