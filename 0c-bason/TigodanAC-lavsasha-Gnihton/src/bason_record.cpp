...
void bason_encode_into(const BasonRecord& record, std::vector<std::uint8_t>& out) {
    ...
    const std::uint32_t len_u32 = static_cast<std::uint32_t>(record.value.size());
    const std::size_t vsize = record.value.size();
    const std::size_t start = out.size();
    out.resize(start + kStubHeaderSize + vsize);
    std::uint8_t* p = out.data() + start;
    p[0] = kStubWireTag;
    p[1] = static_cast<std::uint8_t>(len_u32 & 0xFFu);
    p[2] = static_cast<std::uint8_t>((len_u32 >> 8) & 0xFFu);
    p[3] = static_cast<std::uint8_t>((len_u32 >> 16) & 0xFFu);
    p[4] = static_cast<std::uint8_t>((len_u32 >> 24) & 0xFFu);
    if (vsize != 0) {
        std::memcpy(p + kStubHeaderSize, record.value.data(), vsize);
    }
}
