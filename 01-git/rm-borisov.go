...
func splitTreeLine(treeLine string)(string, string) {
    ...
    if len(splittedTreeLine) != 2 {
        fmt.Fprintf(os.Stderr,
            "Error: the tree line '%s' doesn't contain exactly 1 '\\t' char.\n",
            treeLine)
        os.Exit(1)
    }
    return splittedTreeLine[0],
    splittedTreeLine[1]
}

func createTreeLine(objectName string, objectSha256Hash string) string {
    return strings.Join([] string {
        objectName, objectSha256Hash
    }, "\t")
}
...
func getObjectIndexInTree(sortedTreeLines[] string, objectName string) int {
    ...
    treeLineName,
    _: = splitTreeLine(sortedTreeLines[objectIndex])
...
func removeObjectFromTreeLinesByIndexRecursively(sortedTreeLines[] string,
    objectLineIndex int,
    nextObjectsNamesPath[] string)[] string {
    treeLineName, treeLineHash: = splitTreeLine(sortedTreeLines[
        objectLineIndex])
    switch lastObjectNameChar: = treeLineName[len(treeLineName) - 1];
    lastObjectNameChar {
        case '/':
            if len(nextObjectsNamesPath) > 0 {
                newSubtreeSha256Hash: = removeObjectFromTreeRecursively(
                    treeLineHash,
                    nextObjectsNamesPath)
                sortedTreeLines[objectLineIndex] = createTreeLine(
                    treeLineName, newSubtreeSha256Hash)
            }
        case ':':
            if len(nextObjectsNamesPath) > 0 {
                fmt.Fprintf(os.Stderr,
                    "Error: the %s object is not a tree.\n",
                    treeLineName)
                os.Exit(1)
            }
        default:
            fmt.Fprintf(os.Stderr,
                "Error: the %s object name doesn't end with ':' or '/'.\n",
                treeLineName)
            os.Exit(1)
    }
    if len(nextObjectsNamesPath) == 0 {
        sortedTreeLines = removeFromSliceByIndex(sortedTreeLines,
            objectLineIndex)
    }
...
func removeObjectFromTreeRecursively(treeSha256Hash string, objectsNamesPath[] string) string {
    ...
    objectLineIndex: = getObjectIndexInTree(sortedTreeLines,
        objectsNamesPath[0])
...
func getObjectsNames(objectPath string)[] string {
    ...
    for _,
    objectName: = range objectNames {
        if objectName == "" {
            fmt.Fprintf(os.Stderr,
                "Error: some of the object names is an empty string.\n"
            )
            os.Exit(1)
        }
    }
...
func verifyArgsLength(args[] string) {
    if len(args) != 3 {
        fmt.Fprintf(os.Stderr,
            "Usage: %s object_path root_tree_sha256_hash\n", args[0]
        )
        os.Exit(1)
    }
}
...
func verifyObjectPath(objectPath string) {
    for _, ch: = range objectPath {
        if !isObjectPathCharValid(ch) {
            fmt.Fprintf(os.Stderr,
                "Error: the object path contains the invalid character '%c'.\n",
                ch)
            os.Exit(1)
        }
    }
