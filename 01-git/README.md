...
эту запись нужно обновлять. Дополнительные команды:

 1. commit: принимает текст комментария в stdin, сохраняет его как
    `.message`, запись `.commit` указывает на предыдущий коммит
    (перепрыгивая, возможно, через несколько .parent - если в них 
    .message не обновлялся, версии считаются промежуточными).
    Собственно содержимое команда не меняет (как put, rm...)
 2. log: показывает историю коммитов (хэши и комментарии)
